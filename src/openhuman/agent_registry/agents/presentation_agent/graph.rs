//! LangGraph-compatible execution chain for the `presentation_agent` built-in agent.
//!
...
/// The `presentation_agent` agent's execution chain.
pub fn graph() -> GraphBlueprint {
    blueprint::canonical_turn("presentation_agent")
}
