...
const CONNECTION_ID = 'c-jira-1';
const ACTION = 'JIRA_LIST_ISSUES';
...
async function bootSkillsPage(page: Page, userId: string) {
  ...
  const tile = page.getByTestId('skill-install-composio-jira');
...
async function openModal(page: Page) {
  ...
  const dialog = page.getByRole('dialog', { name: /(Connect|Manage|Reconnect) Jira/i });
...
test.describe('Jira connector', () => {
  test.beforeEach(async ({ page }, testInfo) => {
    ...
    await bootSkillsPage(page, 'pw-jira-' + testSlug);
  });

  test('renders the connector card', async ({ page }) => {
    await expect(page.getByTestId('skill-install-composio-jira')).toContainText(CONNECTOR_NAME);
  });

  test('shows the required Atlassian subdomain input in connect mode', async ({ page }) => {
    await setMockBehavior({
      composioToolkits: JSON.stringify([TOOLKIT_SLUG]),
  ...
  test('routes authorize with subdomain extra params', async () => {
    await callCoreRpc('openhuman.composio_authorize', {
  ...
  test('shows reconnect UI for expired auth without logging out', async ({ page }) => {
    ...
    await reloadSkills(page);
    const dialog = await openModal(page);
    await expect(dialog.getByRole('button', { name: /Reconnect Jira/i })).toBeVisible();
    await assertSessionNotNuked(page);
  });
