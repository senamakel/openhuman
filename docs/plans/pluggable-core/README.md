...
restructures the *host*, not the engines: `tinyagents` / `tinycortex` seams
are untouched.
...
| ----- | ----- | -------------- |
| Transport-agnostic dispatch | `ControllerSchema` + `invoke_method` (`src/core/jsonrpc.rs:230`) shared by HTTP `/rpc`, CLI `call`, generic namespace dispatch, and MCP stdio | The *contract* is host-neutral; only the bootstrap is not |
...
| Headless server mode | `openhuman-core run/serve` (`src/core/cli.rs:66`), `--jsonrpc-only`, Bearer token via `OPENHUMAN_CORE_TOKEN` (`src/core/auth.rs:160`) | Cloud deployment of a *single* core already works |
...
division and this plan keeps it: **we make the core *hostable by* a team
...
- The blockers to in-process tenancy are real and slow: env-var mutation for
  ...
  subscribers (#8), Sentry. Process isolation also gives *security* isolation
...
- **Stage A (facade):** `CoreContext` *owns initialization order* and hands
...
Backend-level pluggability also requires the *stores* to be swappable, not
...
- **Fleet hosting (phase 4)** works *without* abstraction — each per-user core
...
behavior, byte-for-byte). Store traits are carved per domain *as that domain
migrates onto the context* (phase 2 drift ledger gets a "trait extracted?"
...
*host-owned* stores (people, attachments, cost/x402 ledgers, config state,
...
- **Security policy semantics**: `security::live_policy`, approval gate,
  autonomy tiers — unchanged (only *who installs them* moves into
  `CoreBuilder::build`).
...
- **Not attempted**: replacing axum, changing controller schema format,
  in-process tenant *security* isolation, `inventory`/linkme distributed
