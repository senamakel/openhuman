...
const PermissionsSection = ({
  ...
  return (
    <section className="space-y-3">
      <h3 className="text-sm font-semibold text-stone-900">{t('settings.screenIntel.permissions.title')}</h3>
      <PermissionBadge label={t('settings.screenIntel.permissions.screenRecording')} value={screenRecording} />
      <PermissionBadge label={t('settings.screenIntel.permissions.accessibility')} value={accessibility} />
      <PermissionBadge label={t('settings.screenIntel.permissions.inputMonitoring')} value={inputMonitoring} />
      ...
      <button
        ...
        {isRequestingPermissions ? t('settings.screenIntel.permissions.requesting') : t('settings.screenIntel.permissions.requestScreenRecording')}
      </button>
      <button
        type="button"
        ...
        {isRequestingPermissions ? t('settings.screenIntel.permissions.requesting') : t('settings.screenIntel.permissions.requestAccessibility')}
      </button>
      <button
        type="button"
        ...
        disabled={isRequestingPermissions || isRestartingCore}
        className="rounded-lg border border-primary-400 bg-primary-50 px-3 py-2 text-sm text-primary-700 disabled:opacity-50">
        {isRequestingPermissions ? t('settings.screenIntel.permissions.requesting') : t('settings.screenIntel.permissions.openInputMonitoring')}
      </button>
      {anyPermissionDenied ? (
        <button
          type="button"
          ...
          {isRestartingCore ? t('settings.screenIntel.permissions.restartingCore') : t('settings.screenIntel.permissions.restartRefresh')}
        </button>
      ) : (
        <button
          type="button"
          ...
          {isLoading ? t('settings.screenIntel.permissions.refreshing') : t('settings.screenIntel.permissions.refreshStatus')}
        </button>
      )}
    </section>
  );
