...
| P0-4 | `invoke_stream` adoption in `src/openhuman/tinyagents/model.rs` | **OPEN** | The plan calls this out explicitly; the current host still needs a seam audit before switching from manual stream projection. |
| P0-5 | SHA-256 prompt fingerprint / prompt-cache drift guard | **OPEN** | Requires comparing `src/openhuman/tinyagents/middleware.rs` cache guard behavior with TinyAgents 1.6 `PromptCacheGuardMiddleware`. |
| P0-6 | Idempotent redaction middleware vs `journal.rs` double-redaction | **OPEN** | Requires a focused seam audit of `src/openhuman/tinyagents/journal.rs` against TinyAgents 1.6 redaction behavior. |
