...
pub async fn execute_tron_quote(mut quote: PreparedTransaction) -> Result<ExecutionResult, String> {
    ...
    let derived_addr = crate::modules::wallet::derive_account(&config, &signing_secret)
        .await
    ...
    let signed =
        crate::modules::wallet::sign_transaction_in_module(&config, &transaction, &signing_secret)
            .await
