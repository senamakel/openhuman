...
//!   the harness registry of built-in ([`load_builtins`](crate::openhuman::agent_registry::agents::load_builtins)-parsed
//!   `agent.toml`)
//!   and workspace-override agent definitions, plus the config-backed
//!   user-authored fallback
//!   ([`crate::openhuman::agent_registry::find_custom_in_config`] →
//!   [`crate::openhuman::agent_registry::definition_from_registry_entry`]).
//! * [`crate::openhuman::profiles::AgentProfile`] — the active personality,
...
//! [`validate_tier_hierarchy`](crate::openhuman::agent_registry::agents::validate_tier_hierarchy) explicitly `continue`s past unknown ids). So
...
//! [`validate_tier_hierarchy`](crate::openhuman::agent_registry::agents::validate_tier_hierarchy) walks at boot — per declared pair. A `Worker`
//! parent yields an empty list; a tier-illegal child is dropped. Unknown child
//! ids are **kept**, matching both [`validate_tier_hierarchy`](crate::openhuman::agent_registry::agents::validate_tier_hierarchy)'s `continue` and
...
use crate::openhuman::agent_registry::{definition_from_registry_entry, find_custom_in_config};
use crate::openhuman::config::Config;
use crate::openhuman::profiles::AgentProfile;
...
impl OpenHumanDefinitionRegistry {
    ...
    /// — the single source of truth [`validate_tier_hierarchy`](crate::openhuman::agent_registry::agents::validate_tier_hierarchy) walks at boot —
...
mod tests {
    ...
    async fn authorized_delegates_agree_with_boot_time_validation() {
        ...
        let builtin_defs = crate::openhuman::agent_registry::agents::load_builtins()
            .expect("built-in TOML must parse");
        crate::openhuman::agent_registry::agents::validate_tier_hierarchy(&builtin_defs)
    ...
    fn profile_allowing(tools: &[&str]) -> Arc<AgentProfile> {
        let mut profile = crate::openhuman::profiles::built_in_profiles()
            .into_iter()
            .next()
