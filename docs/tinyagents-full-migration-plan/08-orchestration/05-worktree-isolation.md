...
tools, and shell/git plus read-only filesystem tools (`file_read`, `list`) use
`ToolExecutionContext.workspace.root` as their effective action directory when
present. For shell this also covers sandbox policy and sandbox cwd. Delete the
legacy task-local only after the remaining acting tools also resolve roots from
the carried crate `WorkspaceDescriptor`.
...
2. In progress: thread `WorkspaceDescriptor` into tool execution. TinyAgents
   ...
   tool adapter, shell, git operations, `file_read`, and `list`. Remaining
   acting tools still need to read it. OpenHuman `SecurityPolicy` remains the
