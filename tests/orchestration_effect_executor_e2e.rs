...
async fn dispatches_device_status_and_rejects_unknown_tools() {
    // `device_status` / unknown tools are not local-execution tools, so the
    // Master-chat exec gate is bypassed regardless of the cycle id.
    let status = dispatch_device_tool("device_status", &json!({}), "c")
        .await
        .expect("ok");
    assert!(status["version"].is_string());
    assert!(status["platform"].is_string());

    assert!(dispatch_device_tool("rm_rf", &json!({}), "c")
        .await
        .is_err());
}
