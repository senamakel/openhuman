...
impl ChatProvider for CloudChatProvider {
    ...
    async fn chat_for_json(&self, prompt: &ChatPrompt) -> Result<String> {
        log::debug!(
            "[memory::chat::cloud] kind={} model={} sys_chars={} user_chars={}",
            prompt.kind,
            self.model,
            prompt.system.len(),
            prompt.user.len()
            ...
            .await
        {
            Ok(text) => {
                log::debug!(
                    "[memory::chat::cloud] response chars={} kind={}",
                    text.len(),
                    prompt.kind
                );
                return Ok(text);
            ...
            Err(e) if is_model_unavailable_error(&e) => {
                log::warn!(
                    "[memory::chat::cloud] model={} unavailable, trying fallbacks",
                    self.model
                );
            }
            Err(e) => {
                log::warn!(
                    "[memory::chat::cloud] model={} failed kind={} err={:#}",
                    self.model,
                    prompt.kind,
                    e
                );
        ...
        for &fallback in FALLBACK_MODELS {
            ...
            log::debug!("[memory::chat::cloud] trying fallback model={}", fallback);
            match self
                ...
                .await
            {
                Ok(text) => {
                    log::info!(
                        "[memory::chat::cloud] fallback model={} succeeded kind={}",
                        fallback,
                        prompt.kind
                    );
                    return Ok(text);
                ...
                Err(e) if is_model_unavailable_error(&e) => {
                    log::debug!(
                        "[memory::chat::cloud] fallback model={} also unavailable",
                        fallback
                    );
                    continue;
                }
                Err(e) => {
                    log::warn!(
                        "[memory::chat::cloud] fallback model={} failed kind={} err={:#}",
                        fallback,
                        prompt.kind,
                        e
                    );
        ...
        log::warn!(
            "[memory::chat::cloud] configured model={} and all fallbacks unavailable kind={}",
            self.model,
            prompt.kind
        );
        anyhow::bail!(
