    ...
    async fn handle(&self, event: &DomainEvent) {
        match event {
            DomainEvent::SystemStartup { component } => {
                crate::platform::health::mark_component_ok(component);
            }
            DomainEvent::HealthChanged {
                ...
                if *healthy {
                    ...
                    crate::platform::health::mark_component_error(
                        component,
            ...
            DomainEvent::HealthRestarted { component } => {
                crate::platform::health::bump_component_restart(component);
            }
            DomainEvent::ChannelConnected { channel } => {
                crate::platform::health::mark_component_ok(&format!("channel:{channel}"));
            }
