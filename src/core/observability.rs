...
///
/// Note: unlike [`is_transient_provider_http_failure`], this filter is
/// intentionally **not** scoped to `domain == "llm_provider"`. Budget-exhausted
/// 400s from any subsystem (backend API, provider, agent) should be dropped
/// as user-state. The `before_send` budget filter is defense-in-depth over the
/// primary emit-site classifiers; widening the domain scope ensures future call
/// sites are covered without requiring per-domain updates here.
