...
const CONNECTION_ID = 'c-github-1';
const ACTION = 'GITHUB_LIST_REPOS';
...
async function seedConnector(status: 'ACTIVE' | 'FAILED' | 'EXPIRED' = 'ACTIVE') {
  await setMockBehavior({
    ...
    composioAvailableTriggers: JSON.stringify([{ slug: 'GITHUB_COMMIT_EVENT', scope: 'static' }]),
...
async function openModal(page: Page) {
  await page.getByTestId('skill-install-composio-github').click();
  const dialog = page.getByRole('dialog', { name: /(Connect|Manage|Reconnect) GitHub/i });
...
function unwrapTriggerSlugs(payload: unknown): string[] {
  const root = payload as {
    result?: { triggers?: Array<{ slug?: string }> };
    triggers?: Array<{ slug?: string }>;
  };
  const triggers = root.result?.triggers ?? root.triggers ?? [];
  return triggers.map(trigger => trigger.slug).filter((slug): slug is string => Boolean(slug));
}

test.describe('GitHub connector', () => {
  test.beforeEach(async ({ page }, testInfo) => {
    const testSlug = testInfo.title.toLowerCase().replace(/[^a-z0-9]+/g, '-');
    await bootSkillsPage(page, 'pw-github-' + testSlug);
  });

  test('renders the connector card', async ({ page }) => {
    await expect(page.getByTestId('skill-install-composio-github')).toContainText(CONNECTOR_NAME);
  });
