...
const pt4: TranslationMap = {
  ...
  'conversations.toolTimeline.turn': 'turno ${subagent.childIteration}/${subagent.childMaxIterations}',
  ...
  'intelligence.memoryChunk.scoreBars.ariaScore': '${sig.name} pontuação ${(pct * 100).toFixed(0)} por cento',
  ...
  'settings.autocomplete.completionStyle.acceptedCompletion': '${String(historyEntries.length)} conclusão aceita${historyEntries.length === 1 ? \'\' : \'s\'} armazenada — usada para personalizar sugestões futuras.',
  'settings.autocomplete.completionStyle.acceptedCompletions': '${String(historyEntries.length)} conclusão aceita${historyEntries.length === 1 ? \'\' : \'s\'} armazenada — usada para personalizar sugestões futuras.',
