...
//!
//! Mirrors the pattern used by [`overlay::bus`](crate::desktop::overlay::bus)
...
impl NotificationBridgeSubscriber {
    ...
    async fn should_announce(&self, event: &DomainEvent) -> Announce {
        ...
        let snapshot = match crate::config::active_workspace_snapshot().await {
...
pub fn event_to_notification(event: &DomainEvent) -> Option<CoreNotificationEvent> {
    ...
    notification.workspace = event.workspace_dir().map(crate::config::workspace_handle);
    Some(notification)
}
