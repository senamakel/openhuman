...
| Local runtimes (Ollama/LM Studio/MLX/OMLX/local-openai) | `crate_openai::make_crate_local_runtime_chat_model` (native tools + vision forced off; `num_ctx` baked as `{"options":{"num_ctx":N}}`) | **BUILDER READY** — depends on crate toggles ([tinyagents#49](https://github.com/tinyhumansai/tinyagents/pull/49), pinned). **Live dispatch flip deferred**: `create_chat_provider_from_string`'s local branches run the `verify_session_active` gate (custom/local providers require an active OpenHuman session); a crate-native short-circuit in `create_chat_model` **must** re-run that gate or it is a security regression. The gate is `#[cfg(not(test))]`, so neither unit tests nor CI compile-checks cover a mistake — the flip needs a verified host build + reasoning, landed as its own tightly-scoped change. |
