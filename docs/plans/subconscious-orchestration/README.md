...
(`claude "implement docs/plans/subconscious-orchestration/stage-02-core-ingest.md"`) and it has the
...
Stage 1 lands in the **`tiny.place/` checkout** (separate repo, separate PR); stages 2–7 land here.
...
- Only two things live **outside** the graph:
  1. **Transport/ingest** (stage 2): DMs arrive asynchronously; the subscriber persists + then
     *invokes* the graph on the session's thread. A polling node inside the graph would fight
     checkpointing.
  2. **Subconscious** (stage 5): out-of-band cron by design (the spec runs it via
...
Consequence for the stages: stage 3 builds the **graph skeleton + state + front-end nodes**,
stage 4 adds the **reasoning/memory nodes to the same graph**. Different model tiers per node are
...
Claude Code / Codex instance
  └─ tinyplace wrapper (stage 1) — tails session JSONL → SessionEnvelope
       └─ Signal E2E DM → owner agent's tiny.place inbox           [tagged: sessionId, source, role, kind]
            └─ core ingest (stage 2) — orchestration::ingest normalizes DMs → SessionState
                 └─ Front-End Agent (stage 3, Quick LLM) — macro-instructions
                      └─ Reasoning orchestrator graph (stage 4) — sub-agents, 20:1 compression,
                         world-state diff, context hooks; replies upstream
                 └─ Front-End Agent pass 2 — channel_response → Signal DM back to session
            └─ Subconscious tick (stage 5, cron/heartbeat) — reads compressed history + world diff,
               writes steering directives → injected into Reasoning prompts next run
  UI: TinyPlaceOrchestrationTab (stage 6) — master / subconscious / per-session chat windows
```
...
| --- | --- | --- | --- |
| [1 — tiny.place session bridge](stage-01-tinyplace-session-bridge.md) | `tiny.place/` | `SessionEnvelope` v1 schema; Claude Code wrapper; DM forwarding of all user+agent messages | — |
| [2 — core ingest + session state](stage-02-core-ingest.md) | openhuman | `orchestration` domain: DM ingest, session store, typed envelopes | 1 |
| [3 — graph skeleton + front-end nodes](stage-03-frontend-agent.md) | openhuman | Unified `OrchestrationState` graph, checkpointing, two-pass frontend nodes (Quick LLM) | 2 |
| [4 — reasoning + memory nodes](stage-04-reasoning-orchestrator.md) | openhuman | `execute`/`compress`/`world_diff`/`context_guard` nodes in the same graph, sub-agent spawning | 3 |
| [5 — subconscious steering loop](stage-05-subconscious-steering.md) | openhuman | Steering directives from world diff via existing heartbeat/cron | 4 |
| [6 — RPC + UI wiring](stage-06-rpc-and-ui.md) | openhuman | `orchestration.*` RPC; `TinyPlaceOrchestrationTab` on real data + streaming + composer | 2, 5 |
| [7 — testing & observability](stage-07-testing-observability.md) | both | json_rpc_e2e, Vitest, debug logging audit, coverage gate | all |
