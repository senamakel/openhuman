...
const CONNECTION_ID = 'c-googledrive-1';
const ACTION = 'GOOGLEDRIVE_LIST_FILES';
...
test.describe('Google Drive connector', () => {
  test.beforeEach(async ({ page }, testInfo) => {
    const testSlug = testInfo.title.toLowerCase().replace(/[^a-z0-9]+/g, '-');
    await bootSkillsPage(page, 'pw-googledrive-' + testSlug);
  });
