    ...
    <>
      <section className="bg-white dark:bg-neutral-900 rounded-2xl shadow-soft border border-stone-200 dark:border-neutral-800 p-6 space-y-5">
        <div className="space-y-2">
          <h2 className="text-2xl font-semibold text-stone-900 dark:text-neutral-100">{t('rewards.coupon.title')}</h2>
          <p className="max-w-2xl text-sm text-stone-600 dark:text-neutral-300">{t('rewards.coupon.subtitle')}</p>
        </div>
        ...
        <div className="rounded-xl border border-primary-100 bg-primary-50/40 p-4 space-y-3">
          <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
            <input
              ...
              className="flex-1 px-4 py-2.5 rounded-xl border border-stone-200 dark:border-neutral-800 bg-white dark:bg-neutral-900 font-mono text-stone-900 dark:text-neutral-100 placeholder:text-stone-400 dark:placeholder:text-neutral-500 focus:outline-none focus:ring-2 focus:ring-primary-500/40"
            />
            <button
              type="button"
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between gap-3">
            ...
            <button
              ...
              onClick={() => void loadCouponState()}
              disabled={loading}
              className="text-xs font-medium text-stone-500 dark:text-neutral-400 transition-colors hover:text-stone-700 dark:hover:text-neutral-200 dark:text-neutral-200 disabled:opacity-50">
              {t('common.refresh')}
            </button>
          </div>

          {loading && redeemedCoupons.length === 0 ? (
            <p className="text-sm text-stone-500 dark:text-neutral-400">{t('rewards.coupon.loadingHistory')}</p>
          ) : null}
                ...
                <tbody className="divide-y divide-stone-100 dark:divide-neutral-800">
                  {redeemedCoupons.map(coupon => (
                    <tr
                      ...
                      <td className="px-3 py-2 font-mono text-stone-800 dark:text-neutral-100">{coupon.code}</td>
                      <td className="px-3 py-2 text-stone-700 dark:text-neutral-200">{formatUsd(coupon.amountUsd)}</td>
