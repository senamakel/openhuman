...
//!
//! `crate::memory::api` is the vocabulary three parties speak:
//! the host call sites, [`crate::modules::memory::ModuleMemoryProvider`]
...
//! crate into `crates/openhuman-core/src/memory/api/` as 10,894 lines of verbatim copy.
...
fn chunk_types_are_the_contract_crates() {
    fn source_kind(
        k: crate::memory::api::chunks::SourceKind,
    ) -> tinymemory_api::chunks::SourceKind {
        k
    }
    fn chunk(c: crate::memory::api::chunks::Chunk) -> tinymemory_api::chunks::Chunk {
        c
    }
...
fn error_and_wire_table_are_the_contract_crates() {
    fn error(e: crate::memory::api::error::MemoryError) -> tinymemory_api::error::MemoryError {
        e
    }
    let _ = error as fn(_) -> _;

    // Same input, same name, from both ends of the seam.
    let host = crate::memory::api::wire::wire_name(
        &crate::memory::api::error::MemoryError::PathEscape("/etc/passwd".to_string()),
    );
...
fn provider_contract_is_the_contract_crates() {
    fn provider(
        p: std::sync::Arc<dyn crate::memory::api::provider::MemoryProvider>,
    ...
    fn capabilities(
        c: crate::memory::api::capabilities::Capabilities,
...
fn host_seam_types_are_the_contract_crates() {
    fn event(e: crate::memory::api::host::MemoryEvent) -> tinymemory_api::host::MemoryEvent {
        e
    }
    let _ = event as fn(_) -> _;

    fn spacy(s: crate::memory::api::host::SpacyResponse) -> tinymemory_api::host::SpacyResponse {
        s
    }
    let _ = spacy as fn(_) -> _;

    fn connection(
        c: crate::memory::api::host::ComposioConnection,
    ...
    fn execute(
        r: crate::memory::api::host::ComposioExecuteResponse,
    ) -> crate::integrations::composio::types::ComposioExecuteResponse {
        r
    }
...
fn contract_version_is_the_contract_crates() {
    assert_eq!(
        crate::memory::api::CONTRACT_VERSION,
...
fn the_fallback_driver_is_the_contract_crates_but_not_contract_surface() {
    ...
    ) -> std::sync::Arc<dyn crate::memory::api::provider::MemoryProvider> {
        p
    }
    let _ = provider as fn(_) -> _;
}
