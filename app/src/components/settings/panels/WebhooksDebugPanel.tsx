      ...
      <div className="p-4 space-y-4">
        ...
        <div className="flex flex-wrap items-center gap-2 text-xs">
          <button
            ...
            disabled={loading}
            className="rounded-lg border border-stone-200 dark:border-neutral-800 bg-stone-50 dark:bg-neutral-800/60 px-3 py-1.5 font-medium text-stone-700 dark:text-neutral-200 hover:bg-stone-100 dark:hover:bg-neutral-800 dark:bg-neutral-800 disabled:opacity-50">
          ...
          <span className="text-stone-500 dark:text-neutral-400">
            ...
            <span className={isLive ? 'text-sage-600' : 'text-stone-400 dark:text-neutral-500'}>
        ...
        {lastEvent && (
          <div className="text-xs text-stone-500 dark:text-neutral-400">
            Last event: <span className="font-medium text-stone-700 dark:text-neutral-200">{lastEvent.event_type}</span>{' '}
        ...
        <section className="space-y-2">
          <h3 className="text-sm font-semibold text-stone-900 dark:text-neutral-100">Registered Webhooks</h3>
        ...
        {/* Captured Requests */}
        <section className="space-y-2">
          <h3 className="text-sm font-semibold text-stone-900 dark:text-neutral-100">Captured Requests</h3>
          {logs.length === 0 ? (
            <p className="text-xs text-stone-400 dark:text-neutral-500">No webhook requests captured yet.</p>
          ) : (
            <div className="space-y-2">
              {logs.map(entry => (
                <button
                  ...
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <span className="text-[10px] text-stone-500 dark:text-neutral-400">{entry.status_code ?? '...'}</span>
                  </div>
...
function PayloadBlock({ title, value }: { title: string; value: string }) {
  return (
    <details className="text-xs">
      ...
      <pre className="mt-1 max-h-40 overflow-auto rounded-lg border border-stone-200 dark:border-neutral-800 bg-stone-950 p-2 text-[11px] text-stone-100 whitespace-pre-wrap break-words">
        {value}
      </pre>
    </details>
  );
