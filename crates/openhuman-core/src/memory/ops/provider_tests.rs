...
async fn status_without_a_context_reports_an_unresolved_slot() {
    ...
    assert_eq!(
        ...
        crate::core::subsystem::format_contract_version(crate::memory::api::CONTRACT_VERSION)
    );
}
...
async fn bound_driver_status_reports_id_class_contract_and_capabilities() {
    ...
    let cfg = crate::config::schema::MemorySubsystemConfig::default();
    let binding =
        crate::memory::binding::for_workspace(workspace.path(), &cfg).expect("binding resolves");
    ...
    assert_eq!(status.driver, crate::memory::binding::MODULE_ID);
...
async fn a_refused_driver_reports_the_fallback_and_its_reason() {
    let workspace = tempfile::tempdir().expect("tempdir");
    let mut cfg = crate::config::schema::MemorySubsystemConfig {
        driver: "supermemory".into(),
        ..Default::default()
    };
    cfg.drivers.insert(
        "supermemory".into(),
        crate::config::schema::MemoryDriverConfig {
