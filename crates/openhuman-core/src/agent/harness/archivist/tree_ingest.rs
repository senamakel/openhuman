...
use crate::memory::api::chunks::{DataSource, SourceRef};
use crate::memory::api::provider::types::IngestItem;
use crate::memory::api::provider::{ConversationSegment, EpisodicTurn};
    ...
    pub(super) async fn pipe_segment_to_tree(
        ...
        let messages = entries
            ...
            .filter_map(|e| {
                ...
                let (text, _image_refs) = crate::agent::multimodal::parse_image_markers(&raw_text);
                if text.is_empty() {
                    return None;
                }
        ...
        match ingest_result {
            ...
            Err(e) => {
                ...
                if crate::memory::tree::health::user_error::is_corrupt_store_error(&rendered) {
                    tracing::error!(
                        "[archivist] tree ingest hit a CORRUPT memory store: \
                         source_id={source_id} session={session_id} segment={segment_id} \
                         error={rendered}"
                    );
                    crate::memory::tree::health::user_error::notice_corrupt_store_once(
                        "archivist tree ingest",
                    );
                } else if crate::memory::tree::health::user_error::is_local_embedding_error(
                    &rendered,
                ) {
                    tracing::warn!(
                    ...
                    crate::memory::tree::health::user_error::notice_local_model_unavailable_once(
                        "archivist tree ingest",
                    );
                } else {
                    tracing::warn!(
