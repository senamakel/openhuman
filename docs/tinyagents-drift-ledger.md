...
| P0-4 | `invoke_stream` adoption in `src/openhuman/tinyagents/mod.rs` | **OPEN** | `OpenHumanTinyAgentModel::invoke` still calls `invoke_streaming_in_context` when progress streaming is enabled. TinyAgents 1.6 `invoke_stream` yields caller-consumable `AgentStreamItem`s, but OpenHuman's seam currently depends on the existing `RunContext` cancellation, steering, journal, and final `AgentLoopResult` path; needs a dedicated cutover rather than a mechanical rename. |
