...
//! runs inside the shared [`crate::runtime::python_server`] as the
...
use crate::inference::tokenjuice::types::CompressOptions;
...
pub async fn compress(text: &str, _opts: &CompressOptions) -> Result<Option<String>> {
    ...
    let resp = crate::runtime::python_server::request_kompress(&config, text).await?;
