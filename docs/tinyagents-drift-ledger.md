...
| P1-7 | Tool display metadata and timeout semantics | **TIMEOUT SLICE IN REVIEW / DISPLAY RETAINED** | TinyAgents PR [tinyagents#26](https://github.com/tinyhumansai/tinyagents/pull/26) shipped the vocabulary in `v1.7.0`; [tinyagents#70](https://github.com/tinyhumansai/tinyagents/pull/70) now makes `ToolTimeoutSettings` dynamically updateable and enforces `Tool::timeout_policy` in serial and parallel harness execution. The host re-exports crate `ToolTimeout`, pushes env/config precedence into the shared settings, and removes its adapter-level deadline implementation while preserving recoverable tool-error semantics. Display metadata remains part of the later WP-4 tool-model decision. Validation: crate timeout filters + all-target/all-feature Clippy; host root check and timeout-policy filters. |
