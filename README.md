...
- **[Smart token compression (TokenJuice)](https://tinyhumans.gitbook.io/openhuman/features/token-compression)**: every tool call, scrape result, email body, and search payload is run through a compaction layer before it touches the model. HTML is converted to Markdown, long URLs are shortened, etc... Same information, a fraction of the tokens. Saving you tokens and hence making your LLM calls a lot cheaper.
