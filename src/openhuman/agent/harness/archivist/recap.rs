//! Segment-summarization logic for `ArchivistHook` — the shared single-LLM
//! summarizer feeding the finalize (`on_segment_closed`) path.
//!
//! #4469 item 8: the standalone `rolling_segment_recap` entry point was removed
//! — it had zero callers since the tinyagents migration replaced live
//! compaction with the harness context middlewares.
...
impl ArchivistHook {
    ...
    /// Shared summarize helper — the **single LLM summarizer** used by the
    /// finalize path (`on_segment_closed`).
    ///
