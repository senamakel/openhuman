...
pub fn register_learning_subscribers(workspace_dir: std::path::PathBuf) {
    ...
    register_email_signature_once(&EMAIL_SIG_HANDLE, || {
        crate::agent::learning::extract::signature::register_email_signature_subscriber()
    });
...
fn register_with_memory(
    ...
    let rebuild_trigger = {
        ...
        crate::agent::learning::scheduler::spawn_rebuild_loop(
            Arc::clone(&detector),
            crate::agent::learning::scheduler::DEFAULT_REBUILD_INTERVAL,
            shutdown_rx,
        );
