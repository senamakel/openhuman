...
//! `crate::integrations::composio::ops::providers_ops::run_sync_pass`
...
//! [`crate::integrations::composio::providers::has_native_provider`]
...
//! `crate::integrations::composio::providers` for where each of
...
use crate::integrations::composio::providers::has_native_provider;
...
pub async fn list_sync_targets(config: &Config) -> Result<Vec<SyncTarget>, String> {
    let registry_sources = crate::memory::sources::registry::list_enabled_by_kind(
        crate::memory::sources::SourceKind::Composio,
    )
    .await
    .unwrap_or_default();
...
pub async fn scan_active_sync_targets(config: &Config) -> Result<Vec<SyncTarget>, String> {
    let connections = crate::integrations::composio::ops::composio_list_connections(config)
        .await
