...
| 11.1.9 | Core module RPC surfaces   | RI    | `tests/domain_modules_e2e.rs`, `tests/config_auth_app_state_connectivity_e2e.rs`, `tests/worker_b_domain_e2e.rs`, `tests/worker_c_modules_e2e.rs`; `pnpm test:rust:e2e:coverage` | ✅     | Schema catalog + read/status/mutation JSON-RPC coverage for config, auth/credentials, app_state, connectivity, inference, agent, tools, approval, memory, memory_tree, memory_sync, memory_sources, embeddings, channels, composio, and threads; verifier enforces ≥90% controller-method E2E coverage for these named modules |
