...
//! [`integrations::composio::ops::run_sync_pass`](crate::integrations::composio::ops::run_sync_pass)
...
async fn spawn_manual_sync(requested_connection: Option<String>) -> Result<(), String> {
    ...
    tokio::spawn(async move {
        for target in targets {
            ...
            let outcome = crate::integrations::composio::ops::run_sync_within_budget(
                &config,
                &target.toolkit,
...
/// replacement is
/// [`MemoryMaintenance::queue_stats`](crate::memory::api::provider::MemoryMaintenance::queue_stats),
    ...
    #[cfg(feature = "modules")]
    {
        crate::modules::memory::ModuleMemoryProvider::from_boot_policy()
            ...
            .map_err(|error| {
                ...
                if matches!(
                    error,
                    crate::memory::api::error::MemoryError::Unsupported { .. }
                ) {
