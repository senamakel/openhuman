...
function ConnectionOptionRow({
  ...
  const badge = option.comingSoon ? (
    <span className="px-2 py-0.5 text-[11px] font-medium rounded-full bg-stone-100 dark:bg-neutral-800 text-stone-500 dark:text-neutral-400 border border-stone-200 dark:border-neutral-700">
      {t('connections.comingSoon')}
    </span>
  ) : option.statusLabel ? (
    <span className="px-2 py-0.5 text-[11px] font-medium rounded-full bg-sage-50 dark:bg-sage-500/15 text-sage-700 dark:text-sage-300 border border-sage-200 dark:border-sage-500/30">
      {option.statusLabel}
    </span>
  ) : (
  ...
  return (
    <button
      ...
      className={`group w-full flex items-center justify-between p-4 bg-white dark:bg-neutral-900 text-left transition-colors duration-150 ${
        ...
        isDisabled ? 'opacity-70 cursor-not-allowed' : 'hover:bg-stone-50 dark:hover:bg-neutral-800/60 focus-visible:bg-stone-50 dark:focus-visible:bg-neutral-800/60'
      ...
      <div className="flex-1 min-w-0">
        <div className="font-medium text-sm text-stone-900 dark:text-neutral-100 leading-snug">{option.name}</div>
          ...
          {walletConfigured && walletStatus ? (
            <div className="rounded-2xl border border-stone-200 dark:border-neutral-800 bg-white dark:bg-neutral-900 p-4 space-y-3">
              <div>
                ...
                <p className="text-xs text-stone-500 dark:text-neutral-400 mt-1">{t('connections.walletDerived')}</p>
              </div>
