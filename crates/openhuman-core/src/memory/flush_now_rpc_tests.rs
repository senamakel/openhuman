...
async fn flush_now_reports_the_drivers_outcome() {
    use crate::memory::api::provider::types::FlushOutcome;
    ...
    crate::memory::binding::install_for_test(
        ...
        std::sync::Arc::new(
            crate::memory::binding::FixedDiagnostics::new(Default::default(), Default::default())
                .flushing(FlushOutcome {
                    enqueued: false,
                    stale_buffers: u64::from(u32::MAX) + 7,
                }),
        ) as std::sync::Arc<dyn crate::memory::api::provider::MemoryProvider>,
    );
