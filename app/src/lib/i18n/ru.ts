...
const messages: TranslationMap = {
  ...
  'settings.tokenUsage.compressionDesc': 'Сжатие больших выводов инструментов с учётом содержимого до их попадания в контекст модели.',
  ...
  'settings.tokenUsage.searchDesc': 'Ранжировать вывод grep/поиска по релевантности, сохраняя лучшие совпадения.',
  ...
  'settings.tokenUsage.mlDesc': 'Локальная модель ModernBERT для обычного текста (требуется среда выполнения Python).',
  'settings.tokenUsage.ccrTitle': 'Кэш и восстановление (CCR)',
  'settings.tokenUsage.ccrDesc': 'Сжатые оригиналы кэшируются, чтобы агент мог получить полный текст по запросу.',
  'settings.tokenUsage.ccrEnabled': 'Сохранять оригиналы для восстановления',
  'settings.tokenUsage.ccrEnabledDesc': 'Выгрузить оригинал и добавить маркер извлечения, чтобы ничего не потерялось.',
  'settings.tokenUsage.ccrMinTokens': 'Минимальный размер для кэширования',
  'settings.tokenUsage.ccrMinTokensDesc': 'Кэшировать и сжимать с потерями только результаты, оценённые в это количество токенов или больше.',
