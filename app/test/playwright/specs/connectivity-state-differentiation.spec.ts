...
test.describe('Connectivity state differentiation (issue #1527)', () => {
  ...
  test.skip(
    'shows backend-reconnecting status when backend is unreachable but internet is up',
    async () => {}
  );
  ...
  test.skip(
    'status updates to healthy without reinstall after backend recovers from 503',
    async () => {}
  );

  test.skip(
    'shows core-offline indicator (not device-offline) when internet is up but core is unreachable',
    async () => {
