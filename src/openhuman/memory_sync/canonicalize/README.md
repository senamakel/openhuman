...
- [`chat.rs`](../../../../vendor/tinycortex/src/memory/ingest/canonicalize/chat.rs) — chat transcripts (Slack / Discord / Telegram / WhatsApp) → Markdown of `## <ts> — <author>\n<body>` blocks. Sorts messages and captures `time_range`. Produces empty-input `Ok(None)`.
- [`document.rs`](../../../../vendor/tinycortex/src/memory/ingest/canonicalize/document.rs) — single documents (Notion page, Drive doc, meeting note, uploaded file) → trimmed body Markdown. `time_range` collapses to a single point at `modified_at`.
- [`email.rs`](../../../../vendor/tinycortex/src/memory/ingest/canonicalize/email.rs) — email threads (Gmail + generic) → per-message `---\nFrom: …\nSubject: …\nDate: …\n\n<cleaned-body>` blocks. Bodies pass through `email_clean::clean_body` first.
- [`email_clean.rs`](../../../../vendor/tinycortex/src/memory/ingest/canonicalize/email_clean.rs) — pure-string helpers: `clean_body` (strip reply chains + footer/legal boilerplate), `truncate_body`, `md_escape`, `extract_email`, `parse_message_date`. Used by both the email canonicaliser and the `gmail-fetch-emails` bin.
