...
use crate::openhuman::memory_tree::jobs::store;
        ...
        if let Some(content_path) = chunk_store::get_chunk_content_path(config, &chunk.id)? {
            ...
            if let Err(e) = content_tags::update_chunk_tags(&abs_path, &obsidian_tags) {
                log::warn!(
                    ...
                    chunk.id,
                    crate::openhuman::memory_tree::util::redact::redact(&content_path),
                );
...
async fn handle_reembed_backfill(config: &Config, job: &Job) -> Result<JobOutcome> {
    ...
    if chunk_ids.is_empty() && summary_ids.is_empty() {
        crate::openhuman::memory_tree::jobs::set_backfill_in_progress(false);
        log::info!(
...
mod tests {
    ...
    async fn ensure_reembed_backfill_enqueues_only_when_uncovered() {
        use crate::openhuman::memory_tree::jobs::ensure_reembed_backfill;
        use crate::openhuman::memory_tree::store::{upsert_chunks, upsert_staged_chunks_tx};
        use crate::openhuman::memory_tree::types::{
            chunk_id, Chunk, Metadata, SourceKind, SourceRef,
        };
