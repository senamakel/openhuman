...
impl OllamaChatProvider {
    async fn run_chat(&self, prompt: &ChatPrompt, format: Option<&str>) -> Result<String> {
        if self.endpoint.is_empty() || self.model.is_empty() {
            return Err(anyhow!(
                "[memory::chat::local] Ollama endpoint or model not configured \
        ...
        log::debug!(
            "[memory::chat::local] POST {url} kind={} model={} sys_chars={} user_chars={}",
            prompt.kind,
            self.model,
            prompt.system.len(),
            prompt.user.len()
        ...
        log::debug!(
            "[memory::chat::local] ollama response chars={} kind={}",
