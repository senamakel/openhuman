...
fn report_expected_message(kind: ExpectedErrorKind, message: &str, domain: &str, operation: &str) {
    match kind {
        ...
        ExpectedErrorKind::LocalAiCapabilityUnavailable => {
            // Logged at info! (not warn!) intentionally: this is a pure
            // user-state / hardware-tier condition surfaced so the UI can
            // prompt a tier upgrade, not a recoverable runtime failure.
            // Mirrors the LocalAiDisabled precedent for the same reason.
            tracing::info!(
                domain = domain,
                operation = operation,
                error = %message,
...
mod tests {
    ...
    fn classifies_local_ai_capability_unavailable_errors() {
        ...
        for raw in [
            ...
            // Short warning-field form emitted by assets.rs (no trailing sentence).
