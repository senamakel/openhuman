...
fn build_flow_connections_emits_parseable_refs_for_both_kinds() {
    ...
    assert_eq!(
        crate::flows::tinyflows::caps::composio_connection_id(&gmail.connection_ref),
        Some("ca_abc")
    );
    ...
    assert_eq!(
        crate::flows::tinyflows::caps::http_cred_name(&stripe.connection_ref),
        Some("stripe")
    );
...
fn build_flow_connections_attaches_platform_user_id_from_a_seeded_identity() {
    use crate::integrations::composio::providers::ConnectedIdentity;
...
async fn list_suggestions_filters_by_status() {
    ...
    let active = flows_list_suggestions(&config, Some(crate::flows::SuggestionStatus::New))
        .await
        .unwrap();
