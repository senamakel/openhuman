...
const CONNECTION_ID = 'c-discord-1';
const ACTION = 'DISCORD_LIST_SERVERS';
...
async function openModal(page: Page) {
  await page.getByTestId('skill-install-composio-discord').click();
  const dialog = page.getByRole('dialog', { name: /(Connect|Manage|Reconnect) Discord/i });
...
test.describe('Discord connector', () => {
  test.beforeEach(async ({ page }, testInfo) => {
    const testSlug = testInfo.title.toLowerCase().replace(/[^a-z0-9]+/g, '-');
    await bootSkillsPage(page, 'pw-discord-' + testSlug);
  });

  test('renders the connector card', async ({ page }) => {
    await expect(page.getByTestId('skill-install-composio-discord')).toContainText(CONNECTOR_NAME);
  });

  test('does not log the user out when the card is clicked', async ({ page }) => {
    await openModal(page);
    await assertSessionNotNuked(page);
  });

  test('routes authorize through the mock backend', async ({ page }) => {
  ...
  test('persists connected state through list_connections', async ({ page }) => {
