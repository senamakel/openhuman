...
use crate::openhuman::memory_tree::tree_topic::store::{
    distinct_sources_for, get_or_fresh, upsert,
};
use crate::openhuman::memory_tree::tree_topic::types::{
...
pub async fn maybe_spawn_topic_tree(config: &Config, entity_id: &str) -> Result<SpawnOutcome> {
    ...
    // 4. Full recompute.
    run_full_recompute(config, entity_id, &mut counters, now_ms).await
}
...
pub async fn force_recompute(config: &Config, entity_id: &str) -> Result<SpawnOutcome> {
    let now_ms = Utc::now().timestamp_millis();
    let mut counters = get_or_fresh(config, entity_id)?;
    counters.last_updated_ms = now_ms;
    run_full_recompute(config, entity_id, &mut counters, now_ms).await
...
mod tests {
    ...
    use crate::openhuman::memory_tree::tree_topic::store::get;
    ...
    async fn first_ingest_just_bumps_counters() {
        ...
        let out = test_override::with_provider(provider, async {
            maybe_spawn_topic_tree(&cfg, "email:alice@example.com")
                .await
                .unwrap()
        })
        .await;
    ...
    async fn spawn_fires_exactly_once_when_threshold_crossed() {
        ...
        let (out, out2) = test_override::with_provider(provider, async {
            let o1 = force_recompute(&cfg, "email:alice@example.com")
                .await
                .unwrap();
            // Re-running should report TreeExists, NOT a second spawn.
            let o2 = force_recompute(&cfg, "email:alice@example.com")
                .await
                .unwrap();
            (o1, o2)
        })
    ...
    async fn cadence_only_recomputes_every_n_ingests() {
        ...
        let out2 = test_override::with_provider(provider, async {
            let o1 = maybe_spawn_topic_tree(&cfg, "email:alice@example.com")
                .await
                .unwrap();
            assert_eq!(o1, SpawnOutcome::CountersBumped);
            // No tree yet — cadence not crossed after first call.
            assert!(
                existing_topic_tree(&cfg, "email:alice@example.com")
                    .unwrap()
                    .is_none(),
                "no topic tree should exist before cadence is crossed"
            );
