//! LangGraph-compatible execution chain for the `desktop_control_agent` built-in agent.
//!
...
/// The `desktop_control_agent` agent's execution chain.
pub fn graph() -> GraphBlueprint {
    blueprint::canonical_turn("desktop_control_agent")
}
