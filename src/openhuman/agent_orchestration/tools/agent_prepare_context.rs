        ...
        match run_subagent(definition, &scout_prompt, options).await {
            Ok(outcome) => match &outcome.status {
                SubagentRunStatus::Completed => {
                    ...
                    // Bootstrap this thread's goal from the scout's proposal —
                    // but ONLY when the thread has none yet. The orchestrator
                    // stays authoritative (it sets/replaces via `goal_set`); the
                    // context-gathering path just seeds a goal on the first turn
                    // of a fresh chat so the harness has something to steer
                    // toward. Best-effort — a failure never fails the tool.
                    if let (Some(parent), Some(thread_id)) = (current_parent(), current_thread_id())
                    {
                        if let Some(objective) = Self::parse_proposed_goal(&outcome.output) {
