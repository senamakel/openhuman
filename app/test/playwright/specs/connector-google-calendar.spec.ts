...
const CONNECTION_ID = 'c-googlecalendar-1';
const ACTION = 'GOOGLECALENDAR_LIST_EVENTS';
...
test.describe('Google Calendar connector', () => {
  test.beforeEach(async ({ page }, testInfo) => {
    const testSlug = testInfo.title.toLowerCase().replace(/[^a-z0-9]+/g, '-');
    await bootSkillsPage(page, 'pw-googlecalendar-' + testSlug);
  });
