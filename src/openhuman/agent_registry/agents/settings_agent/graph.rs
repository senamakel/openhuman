//! LangGraph-compatible execution chain for the `settings_agent` built-in agent.
//!
...
/// The `settings_agent` agent's execution chain.
pub fn graph() -> GraphBlueprint {
    blueprint::canonical_turn("settings_agent")
}
