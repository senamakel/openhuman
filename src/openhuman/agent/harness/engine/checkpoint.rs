...
pub(crate) trait CheckpointStrategy: Send + Sync {
    /// Produce the turn's result text after the iteration cap is hit, or return
    /// an error to surface the cap to the caller. `digest` is the accumulated
    /// `tool → outcome` summary of the run so far.
    async fn on_max_iter(&self, digest: &str, max_iterations: usize) -> Result<String>;
}
...
impl CheckpointStrategy for ErrorCheckpoint {
    async fn on_max_iter(&self, _digest: &str, max_iterations: usize) -> Result<String> {
