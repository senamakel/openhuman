...
- [`src/openhuman/memory_tree/tools/mod.rs`](../src/openhuman/memory_tree/tools/mod.rs) — generic `memory_tree_*` tools. Filter by `source_kind: "chat"` or query directly; WhatsApp chat-day transcripts are tagged `whatsapp` so they surface in cross-source flows.
