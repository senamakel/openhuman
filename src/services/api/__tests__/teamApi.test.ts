...
describe("teamApi", () => {
  ...
  describe("getTeams", () => {
    it("should call GET /teams and return data", async () => {
      const teams = [
        { team: { _id: "t1", name: "Team 1" }, role: "ADMIN" },
      ];
    ...
    it("should propagate errors", async () => {
      ...
      await expect(teamApi.getTeams()).rejects.toEqual({
        success: false,
        error: "Unauthorized",
      });
    });
  ...
  describe("createInvite", () => {
    ...
    it("should pass options when provided", async () => {
      ...
      expect(mockPost).toHaveBeenCalledWith("/teams/t1/invites", {
        maxUses: 5,
        expiresInDays: 7,
      });
    });
  ...
  describe("joinTeam", () => {
    it("should call POST /teams/join with code in body", async () => {
      ...
      expect(mockPost).toHaveBeenCalledWith("/teams/join", {
        code: "ABC123",
      });
      expect(result).toEqual(team);
    });
