//! The unified `memory_tree_ingest` RPC: canonicalises a chat/email/document
//! payload into contract items and hands them to the bound driver's `Ingest`
//! family.
...
pub use crate::memory::tree::tree::canonicalize_types::*;
...
///
/// Resolved on `provider()`, the same seam [`super::backfill::store_stats`]
/// and [`super::backfill::queue_stats`] use, and deliberately not on
/// `binding.guard()`: the guard's `Ingest` decorator re-stamps `taint`, and
/// `ExternalSync` — what it stamps whenever a source scope is active — is
/// exactly what the driver's own `validate_ingest_item` refuses for the chunk
/// tier, so routing this path through the guard would fail every ingest
/// issued during a sync.
