...
fn deserialize_params_parses_autonomy_update_with_trusted_roots() {
    ...
    assert_eq!(roots[0].access, crate::security::TrustedAccess::ReadWrite);
    // `access` defaults to Read when omitted.
    assert_eq!(roots[1].access, crate::security::TrustedAccess::Read);
}
...
async fn handle_get_autonomy_settings_returns_current_value() {
    ...
    let _ = crate::config::ops::load_and_apply_autonomy_settings(
        crate::config::ops::AutonomySettingsPatch {
