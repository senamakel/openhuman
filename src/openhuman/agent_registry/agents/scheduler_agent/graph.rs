//! LangGraph-compatible execution chain for the `scheduler_agent` built-in agent.
//!
...
/// The `scheduler_agent` agent's execution chain.
pub fn graph() -> GraphBlueprint {
    blueprint::canonical_turn("scheduler_agent")
}
