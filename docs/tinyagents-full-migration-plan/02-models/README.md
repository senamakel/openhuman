...
3. `03-streaming-reasoning.md` — delete `ThinkingForwarder`.
4. `04-catalog.md` — one catalog for pricing/windows/capabilities.

Done when: `provider/reliable.rs` is deleted, reasoning rides the crate
stream, model resolution is capability-checked pre-dispatch, and pricing/
context-window data has one source.
