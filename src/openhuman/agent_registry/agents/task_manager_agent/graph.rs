//! LangGraph-compatible execution chain for the `task_manager_agent` built-in agent.
//!
...
/// The `task_manager_agent` agent's execution chain.
pub fn graph() -> GraphBlueprint {
    blueprint::canonical_turn("task_manager_agent")
}
