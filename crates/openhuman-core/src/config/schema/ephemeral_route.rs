...
//!
//! So the route rides on [`Config`](crate::config::Config) — which
...
/// [`Config`](crate::config::Config) derives it for the whole struct.
...
pub struct EphemeralRoute {
    ...
    ///
    /// Read back only by
    /// [`lookup_key_for_slug`](crate::inference::provider::factory::lookup_key_for_slug)
...
/// [`provider_for_role`](crate::inference::provider::factory::provider_for_role)
...
pub fn apply(config: &mut crate::config::Config, route: EphemeralRoute) {
    let model = config
        .default_model
        .as_deref()
        .map(str::trim)
    ...
    let Some(model) = model else {
        log::warn!(
            ...
            crate::inference::provider::factory::redact_endpoint(&route.endpoint)
        );
        return;
    };
