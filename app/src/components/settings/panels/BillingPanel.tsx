              ...
              <div className="flex flex-wrap items-center gap-2 text-xs text-stone-500 dark:text-neutral-400">
                {breadcrumbs.map((crumb, index) => (
                  <button
                    ...
                    className="rounded-full border border-stone-200 dark:border-neutral-800 bg-white dark:bg-neutral-900 px-3 py-1 font-medium text-stone-600 dark:text-neutral-300 transition-colors hover:bg-stone-50 dark:hover:bg-neutral-800/60">
                    {crumb.label}
                  </button>
                ))}
              </div>
          ...
          <div className="max-w-xl space-y-4">
            ...
            <div className="flex flex-wrap gap-3">
              ...
              <button
                type="button"
                onClick={navigateBack}
                className="inline-flex items-center rounded-full border border-stone-200 dark:border-neutral-800 bg-white dark:bg-neutral-900 px-4 py-2 text-sm font-semibold text-stone-700 dark:text-neutral-200 transition-colors hover:bg-stone-50 dark:hover:bg-neutral-800/60">
                {t('settings.billing.backToSettings')}
              </button>
            </div>

            {status === 'opening' && (
              <p className="text-xs text-stone-500 dark:text-neutral-400">{t('settings.billing.openingBrowser')}</p>
            )}
            {status === 'idle' && (
              <p className="text-xs text-stone-500 dark:text-neutral-400">{t('settings.billing.browserNotOpen')}</p>
            )}
            {status === 'error' && (
