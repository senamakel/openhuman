...
//! loaded through [`crate::modules`] — and that is behind the
...
{
    crate::modules::connectors::call(config, member, request)
        .await
...
#[cfg(feature = "modules")]
pub async fn reconcile_route_if_loaded(config: &crate::config::Config) -> Result<(), String> {
    crate::modules::connectors::reconcile_route_if_loaded(config).await
}
...
pub async fn call_bare<Reply: serde::de::DeserializeOwned>(
    ...
    crate::modules::connectors::call_bare(config, member)
        .await
        .map_err(|error| normalize_error(member, error))
}
