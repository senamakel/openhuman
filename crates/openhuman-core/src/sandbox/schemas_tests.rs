...
fn scoped_ctx(config: crate::config::Config) -> std::sync::Arc<CoreContext> {
...
async fn handle_status_returns_json() {
    let ctx = scoped_ctx(crate::config::Config::default());
...
fn docker_runtime_config() -> crate::config::Config {
