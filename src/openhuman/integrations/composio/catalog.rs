...
fn resolve_composio_action_scope(
    slug: &str,
) -> Option<tinymemory_core::sync::composio::providers::ToolScope> {
    use tinymemory_core::sync::composio::providers::{
...
pub(crate) async fn probe_tool_output_sample(
    ...
    match resolve_composio_action_scope(slug) {
        Some(tinymemory_core::sync::composio::providers::ToolScope::Read) => {}
        Some(other) => {
            tracing::warn!(
                target: "flows",
                %slug,
...
pub(crate) async fn composio_required_args(config: &Config, slug: &str) -> Option<Vec<String>> {
    let toolkit = tinymemory_core::sync::composio::providers::toolkit_from_slug(slug)?;
