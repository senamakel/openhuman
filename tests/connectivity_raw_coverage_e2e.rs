...
async fn pick_listen_port_falls_back_for_non_openhuman_and_status_fingerprints() {
    ...
    assert!(picked.port > probe.port, "fallback port must be higher than probe port");
    ...
    assert!(picked.port > status_probe.port, "fallback port must be higher than status probe port");
