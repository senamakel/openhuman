...
const LoadingPhaseHint = ({ accountId }: { accountId: string }) => {
  ...
  return (
    <span
      ...
      className="text-[11px] font-medium text-stone-400 dark:text-neutral-500 dark:text-neutral-500">
      {text}
    </span>
  );
};
...
const WebviewHost = ({ accountId, provider }: WebviewHostProps) => {
  ...
  return (
    <div
      ref={ref}
      className="relative h-full w-full overflow-hidden rounded-2xl border border-stone-200 dark:border-neutral-800/70 dark:border-neutral-800/70 bg-stone-100 dark:bg-neutral-800 dark:bg-neutral-800 shadow-soft"
      ...
      {!isTimeout ? (
        <div
          ...
          className={`pointer-events-none absolute inset-0 flex flex-col items-center justify-center gap-3 ${
            isLoading ? 'text-stone-500 dark:text-neutral-400 dark:text-neutral-400' : 'text-stone-400 dark:text-neutral-500 dark:text-neutral-500'
          }`}
          ...
          />
          <span
            className={`text-xs font-medium tracking-wide ${isLoading ? '' : 'text-stone-500 dark:text-neutral-400 dark:text-neutral-400'}`}>
          ...
          {isLoading ? (
            <div
              ...
              <div className="h-8 w-8 animate-spin rounded-full border-2 border-stone-300 dark:border-neutral-700 dark:border-neutral-700 border-t-stone-600 dark:border-t-neutral-400" />
      ...
      {isTimeout ? (
        <div
          ...
          <div className="max-w-sm space-y-1">
            <p className="text-sm font-semibold text-stone-800 dark:text-neutral-100 dark:text-neutral-100">{`${providerName} ${t('accounts.webviewHost.takingLonger')}`}</p>
            <p className="text-xs text-stone-600 dark:text-neutral-300 dark:text-neutral-300">{t('accounts.webviewHost.timeoutHint')}</p>
          </div>
          <button
            type="button"
            onClick={() => {
