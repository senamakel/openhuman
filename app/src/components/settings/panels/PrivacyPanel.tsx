...
const KIND_BADGE_CLASS: Record<PrivacyDataKind, string> = {
  raw: 'bg-sage-50 dark:bg-sage-500/15 text-sage-700 dark:text-sage-300 border-sage-200 dark:border-sage-500/30',
  derived: 'bg-amber-50 dark:bg-amber-500/15 text-amber-700 dark:text-amber-300 border-amber-200 dark:border-amber-500/30',
  credentials: 'bg-stone-100 dark:bg-neutral-800 text-stone-700 dark:text-neutral-200 border-stone-200 dark:border-neutral-700',
  diagnostics: 'bg-primary-50 text-primary-700 border-primary-200',
  metadata: 'bg-stone-50 dark:bg-neutral-800/60 text-stone-600 dark:text-neutral-300 border-stone-200 dark:border-neutral-700',
};
        ...
        <div className="p-4 space-y-4">
          ...
          <div>
            ...
            <div className="bg-white dark:bg-neutral-900 rounded-xl border border-stone-200 dark:border-neutral-800 overflow-hidden">
              {loadState === 'loading' && (
                <p className="p-4 text-xs text-stone-500 dark:text-neutral-400">{t('privacy.loading')}</p>
              )}
              {loadState === 'error' && (
                <p className="p-4 text-xs text-stone-500 dark:text-neutral-400" data-testid="privacy-load-error">
                  {t('privacy.loadError')}
                </p>
              )}
              {loadState === 'ready' && capabilities.length === 0 && (
                <p className="p-4 text-xs text-stone-500 dark:text-neutral-400">{t('privacy.noCapabilities')}</p>
              )}
              {loadState === 'ready' && capabilities.length > 0 && (
                <ul className="divide-y divide-stone-100 dark:divide-neutral-800" data-testid="privacy-capability-list">
                  {capabilities.map(cap => (
                    <li key={cap.id} className="p-4" data-testid={`privacy-row-${cap.id}`}>
                      <div className="flex items-start justify-between gap-3">
                        <div className="flex-1 min-w-0">
                          <p className="text-sm font-medium text-stone-900 dark:text-neutral-100">{cap.name}</p>
