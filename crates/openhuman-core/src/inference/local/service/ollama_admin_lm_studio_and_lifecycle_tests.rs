...
async fn kill_ollama_server_kills_owned_child() {
    ...
    *service.owned_ollama.lock() = Some(child);
    ...
    assert!(
        crate::inference::local::service::spawn_marker::pid_is_alive(pid),
    ...
    for _ in 0..40 {
        if !crate::inference::local::service::spawn_marker::pid_is_alive(pid) {
            still_alive = false;
            break;
        }
...
async fn shutdown_owned_ollama_clears_marker_and_kills_child() {
    ...
    *service.owned_ollama.lock() = Some(child);
    ...
    if marker_writable {
        crate::inference::local::service::spawn_marker::write_marker_at(
            &marker_path,
            &crate::inference::local::service::spawn_marker::OllamaSpawnMarker::new(
                pid,
