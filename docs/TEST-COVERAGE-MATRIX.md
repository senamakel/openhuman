...
| 4.2.7 | Plan-mode review (Approve / Reject / Send-feedback before execute) | RU+RI+VU | `src/openhuman/agent/tools/todo.rs`, `src/openhuman/todos/ops.rs`, `tests/json_rpc_e2e.rs`, `app/src/pages/conversations/components/PlanReviewCard.test.tsx`, `app/src/pages/conversations/taskPlanActions.test.ts`, `app/src/pages/__tests__/Conversations.render.test.tsx` | ✅     | Interactive (`WebChat`) turns default new `todo` cards to approval-required → parked at `awaiting_approval`; `PlanReviewCard` aggregates the plan with Approve-all (`decide_plan`) / Reject / Send-feedback (`todos_revise_plan` → re-plan). RU covers origin gating + `revise_plan`; RI covers the RPC; VU covers the card + actions. WD E2E (agent-driven park flow) tracked as follow-up |
