...
const messages: TranslationMap = {
  ...
  'migration.description': '将记忆和笔记从另一个本地助手迁移到此工作区。先点击「预览」查看将要变更的内容，然后点击「应用」复制数据。当前的记忆会先备份。',
  ...
  'migration.sourceHint': '留空时使用该助手的默认位置。如果你已将工作区移到其他位置，请填写明确路径。',
  ...
  'migration.confirmImport.singular': '将 {count} 条数据导入当前工作区？\n\n来源：{source}\n目标：{target}\n\n导入前会先备份现有记忆。',
  'migration.confirmImport.plural': '将 {count} 条数据导入当前工作区？\n\n来源：{source}\n目标：{target}\n\n导入前会先备份现有记忆。',
  ...
  'welcome.clearAppDataWarning': '此操作将清除此设备上本地存储的密钥和账户。您的云端账户不受影响——清除后可立即重新登录。',
  ...
  'skills.integrationsSubtitle': '基于云端的 OAuth 连接——使用您的账户登录，Composio 代管令牌，让智能体能以您的名义读取数据并执行操作，无需管理 API 密钥。',
  'skills.composio.noApiKeyTitle': '尚未配置 Composio API 密钥',
  'skills.composio.noApiKeyDescription': '本地模式使用你自己的 Composio API 密钥。在此连接集成之前，请打开 设置 → 高级 → Composio 添加一个密钥。',
  ...
  'graphCentrality.intro': '对你的记忆图谱运行 PageRank，可找出关键枢纽以及连接原本分离聚类的连接实体，这是单纯频次统计无法揭示的。',
  ...
  'rewards.coupons': '优惠券',
  'rewards.localUnavailable': '本地登录不会获得奖励、优惠券或推荐积分。若要累计奖励，请先登出，然后使用 OpenHuman 账号登录。',
  ...
  'onboarding.apiKeys.subtitle': '你可以现在粘贴，也可以跳过并稍后在设置 › AI 中添加。密钥会加密存储在此设备上。',
  'onboarding.apiKeys.openaiLabel': 'OpenAI API 密钥',
  'onboarding.apiKeys.openaiPlaceholder': 'sk-...',
  'onboarding.apiKeys.openaiOauthHint': '使用 ChatGPT Plus/Pro（订阅）或 OpenAI API 密钥 — 并非两者都需要。',
  ...
  'onboarding.custom.configureLater': '你可以在入门流程结束后继续配置。完成后，我们会带你前往对应的设置页面。',
  ...
  'onboarding.custom.inference.defaultDesc': 'OpenHuman 会为每种工作负载路由到合适的默认模型。无需密钥，无需设置。',
  'onboarding.custom.inference.configureDesc': '使用你自己的 OpenAI 或 Anthropic 密钥。我们会将它用于所有文本类工作负载。',
  ...
  'onboarding.custom.voice.configureDesc': '使用你自己的 ElevenLabs、OpenAI Whisper 等服务。在设置 › 语音中配置。',
  'onboarding.custom.oauth.title': '连接（OAuth）',
  'onboarding.custom.oauth.subtitle': 'Gmail、Slack、Notion 以及其他需要 OAuth 的连接服务。',
  'onboarding.custom.oauth.defaultDesc': 'OpenHuman 使用托管的 Composio 工作区。之后每个服务都可一键连接。',
  'onboarding.custom.oauth.configureDesc': '使用你自己的 Composio 账户或 API 密钥。在设置 › 连接中配置。',
  ...
  'onboarding.custom.embeddings.configureDesc': '使用您自己的嵌入服务提供商（OpenAI、Voyage、Ollama 等）。',
  ...
  'devOptions.toolPolicyDiagnostics.mcpAllowlists.summary': '已启用：{enabled} · 服务器：{enabledCount}/{totalCount}',
  ...
  'devOptions.toolPolicyDiagnostics.mcpWriteAudit.summary': '已启用：{enabled} · 最近（24 小时）：{recentRows}',
  ...
  'devOptions.toolPolicyDiagnostics.redactedSurfaces.summary': '可写：{writeCount} · 策略表面：{policyCount}',
  ...
  'settings.about.connectionHelperLocal': '应用启动时由 Tauri shell 在进程内启动。端口会在启动时选择，因此此 URL 每次启动都可能变化。',
  ...
  'settings.costDashboard.disabledHint': '成本看板已在配置中禁用。请在 config.toml 中设置 [cost.dashboard] enabled = true 以重新启用。',
  'settings.costDashboard.subtitle': '集群实时支出和 token 消耗。条形图每隔几秒自动刷新，无需重新加载页面。',
  ...
  'settings.costDashboard.budgetLimitHint': '从 config.toml 的 cost.monthly_limit_usd 读取月度预算。',
  ...
  'settings.costDashboard.noDataHint': '发送一条智能体消息后，下一次提供商调用的 token 用量会在约 10 秒内填充图表。',
  'settings.search.title': '搜索引擎',
  'settings.search.menuDesc': '默认使用 OpenHuman 托管搜索，或用 API 密钥接入你自己的提供商。',
  'settings.search.description': '选择智能体使用的搜索引擎。托管模式使用 OpenHuman 后端（无需设置）。Parallel、Brave 和 Querit 会用你的 API 密钥从本机直接调用。',
  ...
  'settings.search.localManagedUnavailable': '本地用户无法使用 OpenHuman 托管搜索。请添加你自己的 Parallel、Brave 或 Querit API 密钥以启用网页搜索。',
  'settings.search.engineParallelLabel': 'Parallel',
  'settings.search.engineParallelDesc': '直接调用 Parallel API：搜索、提取、聊天、研究、增强和数据集工具。',
  ...
  'settings.search.engineQueritDesc': '直接调用 Querit API：支持站点、时间范围、国家/地区和语言筛选的网页搜索。',
  ...
  'settings.search.allowedSitesHint': '助手在研究时可以打开并阅读的网站（每行一个主机，例如 reuters.com）。主机也包含其子域名。留空将阻止所有网页访问。',
  ...
  'settings.embeddings.description': '选择将记忆转换为语义搜索向量的嵌入提供商。更改提供商、模型或维度会使已存储的向量无效，需要完全重置记忆。',
  ...
  'settings.embeddings.wipeBody': '更改嵌入提供商、模型或维度将删除所有已存储的记忆向量。记忆必须重新构建后检索才能再次工作。此操作无法撤消。',
  ...
  'settings.embeddings.vectorSearchDisabled': '向量搜索已禁用。记忆召回将只使用关键词匹配和时间新近度，不进行语义排序。',
  'settings.embeddings.clearKey': '清除 API 密钥',
  'pages.settings.ai.embeddings': '向量嵌入',
  'pages.settings.ai.embeddingsDesc': '用于记忆检索的向量编码模型',
  'mcp.alphaBadge': '阿尔法',
  'mcp.alphaBannerText': 'MCP 服务器支持仍处于早期 alpha 阶段。Smithery 注册表、安装流程和工具接线在不同版本间可能异常或发生变化。',
  ...
  'mcp.setup.secretDialog.privacyNote': '会加密存储在本地 MCP 密钥表中。绝不记录到日志，也不会发送给模型。',
  ...
  'mcp.health.disconnectConfirm.body': '这会断开当前已连接的 {count} 个 MCP 服务器。已安装配置和密钥会保留，你之后可以重新连接任意服务器。',
  ...
  'mcp.inventory.subtitle': '将已安装的 MCP 服务器导出为可移植且不含密钥的 manifest，或从队友处导入。密钥环境值绝不会被包含或导入。',
  ...
  'mcp.inventory.export.privacyBody': '仅包含服务器名称、限定名称、环境变量 KEY 名称和非密钥配置。密钥值、机器标识符和每次安装的时间戳会被有意移除。',
  ...
  'mcp.inventory.import.trustBody': 'MCP 服务器是你授权给智能体的工具。只导入来自可信来源的 manifest。每次安装都需要你明确点击，不会自动安装任何内容。',
  ...
  'mcp.inventory.parseError.unsupportedSchema': '不支持的 manifest schema，此文件不是由兼容的导出器生成的。',
  ...
  'mcp.inventory.parseError.serverEnvKeysNotArray': '某个服务器条目的 env_keys 字段不是字符串数组。',
  'mcp.inventory.parseError.serverContainsEnv': '某个服务器条目包含 `env` 值映射。已拒绝导入，manifest 只能携带 env_keys（名称），绝不能包含密钥值。',
  'mcp.inventory.parseError.duplicateQualifiedName': 'Manifest 中发现重复的 qualified_name。每个服务器最多只能出现一次。',
  ...
  'autonomy.maxActionsHelp': '智能体每滚动小时内可执行的最大工具操作次数。新值将在下次对话时生效。定时任务和频道监听器将保持当前限制，直到重启 OpenHuman。',
  ...
  'voice.providers.desc': '选择转录和合成的运行位置。使用「本地安装」按钮将二进制文件和模型下载到您的工作区。本地服务提供商可在安装完成前保存——无需手动配置 WHISPER_BIN 或 PIPER_BIN。',
  ...
  'voice.providers.piperDownloadTitle': '将 Piper 及内置的 en_US-lessac-medium 声音下载到您的工作区。',
  ...
  'voice.providers.piperVoicesDesc': '声音来自 huggingface.co/rhasspy/piper-voices。切换声音后，可能需要点击「安装/重新安装」以下载新的 .onnx 文件。',
  'voice.providers.mascotVoice': '吉祥物声音',
  'voice.providers.mascotVoiceDescPrefix': '吉祥物语音回复所使用的 ElevenLabs 声音，在以下位置配置：',
  ...
  'voice.modal.whisperDesc': '选择模型大小并将 Whisper 二进制文件和 GGML 模型安装到您的工作区。模型越大，精度越高，但速度越慢。',
  'voice.modal.piperDesc': '选择一个语音并将 Piper 二进制文件和 ONNX 模型安装到您的工作区。Piper 可完全离线运行，延迟极低。',
  ...
  'voice.routing.elevenlabsVoiceDesc': '选择精选声音，或粘贴来自 ElevenLabs 控制台的自定义声音 ID。',
  ...
  'screenAwareness.debug.useVisionModelDesc': '将截图发送给视觉 LLM 以获取更丰富的上下文。关闭时，仅使用 OCR 文本配合纯文本 LLM——速度更快且无需视觉模型。',
  ...
  'notifications.routing.intelligenceDesc': '来自已连接账户的每条通知都由本地 AI 模型进行评分。重要通知将自动路由至您的编排器智能体，确保关键信息不会遗漏。',
  ...
  'workspace.openingVaultMessage': '如果 Obsidian 没有打开，请从 obsidian.md 安装或使用"显示文件夹"。存储库路径：',
  ...
  'workspace.vaultNotRegisteredHelp': 'Obsidian 只能打开您已添加为存储库的文件夹。在 Obsidian 中选择「以存储库方式打开文件夹」并选取下方文件夹——只需操作一次。然后再次点击「查看存储库」。',
  'workspace.obsidianNotFoundHelp': '未能在此设备上找到 Obsidian。请安装它，或者如果安装在非标准位置，请在「高级」中设置其配置文件夹。',
  ...
  'workspace.obsidianConfigDirHint': '包含 obsidian.json 的文件夹路径（例如 ~/.config/obsidian）。留空以自动检测。',
  ...
  'memorySources.noCustomSources': '暂无自定义来源。添加文件夹、GitHub 仓库、RSS 订阅或网页即可开始。',
  ...
  'bootCheck.portConflictBody': '另一个进程正在占用 OpenHuman 所需的网络端口。我们将尝试自动修复此问题。',
  ...
  'composio.expiredDescription': '重新连接以重新启用 {name} 工具。 OpenHuman 将保持此集成不可用，直到您刷新 OAuth 访问权限。',
  ...
  'app.openhumanLink.notifications.sent': '测试通知已发送。如果未收到，请前往「系统设置 → 通知 → OpenHuman」，开启「允许通知」并将横幅样式设置为「持续」。',
  ...
  'channels.telegram.remoteControlBody': '从允许的 Telegram 聊天中，发送 /status、/sessions、/new 或 /help。模型路由仍然使用 /model 和 /models。',
  ...
  'iosPair.instructions': '在桌面端打开 OpenHuman，前往「设置 > 设备」，点击「配对手机」以显示二维码。',
  ...
  'composio.connect.dynamicsOrgNameHint': '例如，myorg.crm.dynamics.com 的组织名称为 "myorg"。仅输入简短的组织名称，而不是完整 URL。',
  ...
  'composio.connect.idleDescriptionSuffix': '账户。我们会打开浏览器窗口，你在其中授权后，应用会自动检测到该连接。',
  ...
  'composio.connect.needsSubdomainSuffix': '请输入你的 Atlassian 子域名（例如 acme.atlassian.net 中的 acme），然后重试。',
  ...
  'composio.connect.permissionsNoteSuffix': 'OpenHuman 自身的智能体权限通过下方的读取、写入和管理员开关控制。',
  ...
  'composio.connect.subdomainInvalid': '仅输入短子域名（例如 "acme"），而非完整 URL。只能包含字母、数字和连字符。',
  'composio.connect.subdomainRequired': '请输入你的 Atlassian 子域名以继续。',
  'composio.connect.wabaIdHint': '通过 Meta 访问令牌调用 GET /me/businesses，然后 GET /{business_id}/owned_whatsapp_business_accounts 获取。',
  ...
  'onboarding.contextGathering.errorDesc': '我们暂时无法构建你的完整资料，但没关系——你可以继续，资料会随时间逐步完善。',
  ...
  'pages.settings.account.migrationDesc': '将 OpenClaw（即将支持 Hermes）的记忆和笔记迁移到此工作区。',
  ...
  'pages.settings.composioSection.description': '由 Composio 提供支持的集成的路由、触发器和历史记录。',
  ...
  'pages.settings.features.desktopCompanionDesc': '具有屏幕感知能力的语音助手 — 倾听、观看、说话、指向',
  ...
  'settings.ai.openAiCompat.description': '将本地测试工具指向此 /v1 服务器，即可通过下方配置的提供商进行路由。身份验证使用你在此处设置的稳定密钥，而不是应用内部的 core bearer。',
  ...
  'skills.mcpComingSoon.description': 'MCP 服务器管理即将推出。此标签页将成为发现、连接和监控 MCP 服务器集成的中心。',
  ...
  'settings.ai.localRuntimeHelper': '{label} 可访问的位置。默认是 localhost；如需使用共享实例，可指向远程主机（例如 http://10.0.0.4:11434/v1）。',
  ...
  'settings.ai.or': '或者',
  'settings.ai.openRouterOauthDescription': '使用 OpenRouter 登录，并通过 PKCE 导入由用户控制的 API 密钥。',
  'settings.ai.connecting': '正在连接...',
  'settings.ai.backgroundLoops': '背景循环',
  'settings.ai.backgroundLoopsDesc': '查看没有聊天消息时运行的内容，暂停心跳任务，并检查最近的额度账本行。',
  ...
  'settings.ai.plannerSummary': '规划器：{sourceEvents} 源事件，{sent} 已发送，{deduped} 已删除重复数据。',
  ...
  'settings.ai.reindexingMemoryMessage': '嵌入正在重新处理。{pending} 个记忆项正使用当前模型重新嵌入；完成前语义召回会降低。关键词搜索仍可使用，即使关闭此页面，重新嵌入也会在后台继续。',
  ...
  'settings.ai.routing.managedDesc': 'OpenHuman 会在云端运行所有推理，为任务选择最佳模型，优化成本，并保持最安全的路由默认值。',
  'settings.ai.routing.managedMsg': 'OpenHuman 会处理每个工作负载的全部推理，并自动选择兼顾成本、质量和安全性的最佳路线。',
  'settings.ai.routing.useYourOwn': '使用您自己的模型',
  'settings.ai.routing.useYourOwnDesc': '选择一个提供商 + 模型，并让每个工作负载都通过它路由。这样更简单，但轻量和重量级推理共用同一路由，可能效率较低。',
  'settings.ai.routing.advanced': '高级',
  'settings.ai.routing.advancedDesc': '为不同任务选择不同模型。这是严格成本优化和最大控制力的最佳选项。',
  'settings.ai.routing.customDesc': '细粒度路由可提供最佳成本优化和最高控制力。使用下方各行决定哪些工作负载保持托管、哪些使用共享默认值、哪些固定到特定模型。',
  ...
  'settings.ai.globalModel.desc': '这会让所有推理都通过一个模型路由。它更简单，但轻量和重型任务都使用同一路由，可能不利于成本和质量。',
  'settings.ai.globalModel.noProviders': '请先添加或连接提供商。然后你可以在此将每个工作负载路由到一个模型。',
  ...
  'settings.ai.globalModel.appliesToAll': '将同一个提供商 + 模型应用于聊天、推理、编码、记忆、心跳、学习和潜意识。嵌入单独配置。点击保存后更改会保存。',
  ...
  'settings.billing.inferenceBudget.noRecurringWeeklyDesc': '你当前的方案不包含每周循环推理预算。用量将改从可用额度中支付。',
  ...
  'settings.billing.inferenceBudget.exhaustedDesc': '订阅包含的用量已耗尽。充值额度即可继续使用 AI，无需等待下个周期。',
  ...
  'settings.composio.confirmItem3': '注意：Composio 触发器（实时 Webhook）在直连模式下尚不可用——仅支持同步工具调用',
  'settings.composio.confirmNeedItems': '你需要：',
  'settings.composio.confirmSwitch': '我已了解，切换到直连',
  'settings.composio.confirmTitle': '⚠️ 切换到直连模式',
  'settings.composio.confirmWarning': '你现有的集成（通过 OpenHuman 关联的 Gmail、Slack、GitHub 等）将不可见——它们存放在 OpenHuman 托管的 Composio 租户中。',
  'settings.composio.intro': 'Composio 集成了 250+ 外部应用作为智能体可调用的工具。请选择这些工具调用的路由方式。',
  'settings.composio.title': 'Composio',
  'settings.composio.modeDirect': '直连（自带 API 密钥）',
  'settings.composio.modeDirectDesc': '调用直接发送到 backend.composio.dev。自主可控 / 适合离线场景。工具同步执行；实时触发器 Webhook 目前在直连模式下尚未路由（后续问题）。',
  'settings.composio.modeManaged': '托管（由 OpenHuman 管理）',
  'settings.composio.modeManagedDesc': 'OpenHuman 通过我们的后端代理工具调用（推荐）。认证由我们处理，你无需粘贴 Composio API 密钥。Webhook 完全路由。',
  ...
  'settings.localModel.deviceCapability.installFirstDesc': '本地层级依赖外部托管的 Ollama 端点。请自行启动它，拉取所需模型，并在运行时可达前继续使用「已禁用（云端回退）」。',
  ...
  'settings.localModel.deviceCapability.presetDetails': '聊天：{chatModel} · 愿景：{visionModel} · 目标 RAM：{targetRamGb} GB',
  ...
  'settings.localModel.download.summaryHelper': '通过 Rust 核心调用 `open human.inference_summarize`',
  ...
  'settings.localModel.status.contextBelowMinimumBadge': '{contextLength} ctx - 低于 {required} 分钟',
  'settings.localModel.status.contextBelowMinimumTitle': '已拒绝：上下文窗口 {contextLength} 令牌低于内存层所需的 {required} 令牌最小值。回忆会因无声截断而被破坏。',
  'settings.localModel.status.contextOkBadge': '{contextLength} ctx ✓',
  'settings.localModel.status.contextOkTitle': '上下文窗口 {contextLength} 令牌满足内存层最小值 {required} 令牌。',
  'settings.localModel.status.contextUnknownBadge': 'ctx 未知',
  'settings.localModel.status.contextUnknownTitle': '上下文窗口未知；无法确认它满足 {required}-token 内存层最小值。',
  'settings.localModel.status.customLocation': '自定义位置',
  'settings.localModel.status.customLocationDesc': '自定义 Ollama 安装路径',
  'settings.localModel.status.diagnosticsHint': '点击「运行诊断」以验证 Ollama 是否正在运行以及模型是否已安装。',
  ...
  'settings.localModel.status.ollamaNotInstalledDesc': 'OpenHuman 现在将 Ollama 视为外部推理运行时。请启动你自己的 Ollama 服务器，拉取所需模型，并将工作负载路由指向它。',
  ...
  'settings.developerMenu.devWorkflow.panelDesc': '配置一个自主开发智能体，按计划选择分配给你的 GitHub issue 并自动发起 pull request。',
  'settings.developerMenu.skillsRunner.title': '技能运行器',
  'settings.developerMenu.skillsRunner.desc': '临时运行任意内置技能 — 填写输入并启动后台自主运行',
  'settings.developerMenu.skillsRunner.panelDesc': '选择内置技能，填写它声明的输入，并启动一次无需等待的后台运行。如果需要按 cron 计划重复运行，请使用开发工作流。',
  ...
  'settings.skillsRunner.schedule.help': '将此技能和输入保存为重复 cron 任务。智能体会在每次触发时调用 run_skill。',
  ...
  'settings.skillsRunner.repoPicker.notConnected': 'GitHub 尚未通过 Composio 连接。请先在“技能 → Composio”下连接。',
  ...
  'settings.devWorkflow.errorNotConnected': 'GitHub 未连接。请先通过“设置 > 高级 > Composio”连接 GitHub。',
  'settings.devWorkflow.errorToolNotEnabled': '此后端未启用 GITHUB_LIST_REPOSITORIES_FOR_THE_AUTHENTICATED_USER 工具。请让管理员在 Composio 集成中启用它（backend#842）。',
  ...
  'settings.developerMenu.composioRouting.desc': '使用你自己的 Composio API 密钥并将调用直接路由到 backend.composio.dev',
  ...
  'settings.mcpServer.toolsSectionDesc': '运行 openhuman-core mcp 时通过 MCP stdio 服务器暴露的工具',
  ...
  'settings.mcpServer.binaryPathNotFound': '未找到 OpenHuman 二进制文件。如果使用源码运行，请执行：cargo build --bin openhuman-core',
  ...
  'settings.agentAccess.tier.readonly.desc': '可读取文件并运行只读命令来探索 — 但绝不会写入、编辑或运行任何会更改状态的操作。',
  'settings.agentAccess.tier.supervised.title': '编辑前询问',
  'settings.agentAccess.tier.supervised.desc': '可自由创建新文件，但在编辑现有文件、运行命令、访问网络或安装任何内容前会请求你的批准。',
  'settings.agentAccess.tier.full.title': '完全访问',
  'settings.agentAccess.tier.full.desc': '使用你的完整用户账户权限运行命令 — 可在允许的位置读写，但凭据和系统存储除外。破坏性命令、网络访问和安装仍会请求批准。',
  'settings.agentAccess.defaultTag': '（默认）',
  'settings.agentAccess.fullWarning': '⚠ 完全访问会使用你的完整账户权限运行命令，且没有沙盒。仅在你信任智能体可使用这台机器时启用。凭据和系统目录仍会被阻止，破坏性、网络和安装操作仍会请求批准。',
  'settings.agentAccess.confine.label': '限制在工作区',
  'settings.agentAccess.confine.desc': '无论选择哪种访问模式，都将智能体限制在工作区目录（以及已授权文件夹）内。关闭后，它可访问你的用户可访问的任何位置 — 始终阻止的凭据和系统目录除外。',
  'settings.agentAccess.requireTaskPlanApproval.label': '要求批准任务计划',
  'settings.agentAccess.requireTaskPlanApproval.desc': '在指定智能体执行由智能体编写的任务简报前暂停。',
  'settings.agentAccess.grantedFolders': '已授权文件夹',
  'settings.agentAccess.alwaysAllow': '始终允许的工具',
  'settings.agentAccess.alwaysAllowDesc': '你在聊天中标记为“始终允许”的工具会直接运行且不再询问。移除后会重新提示。',
  'settings.agentAccess.alwaysAllowNone': '还没有始终允许的工具。',
  'settings.agentAccess.grantedDesc': '除工作区外，智能体可读写的文件夹。凭据存储（~/.ssh、~/.gnupg、~/.aws、keychains）和系统目录（/etc、/System、C:\\Windows、…）始终被阻止，即使它们位于已授权文件夹内。',
  ...
  'settings.appearance.helperText': '深色模式会将整个应用——聊天、设置、面板——切换为暗色调。"跟随系统"会同步你的操作系统外观并实时更新。',
  ...
  'settings.mascot.customGifError': '输入 HTTPS .gif 链接、本地回环 HTTP .gif 链接、file:// .gif 链接或本地 .gif 路径。',
  ...
  'settings.persona.identityDesc': '助手的显示名称和简短描述。会显示在应用中；不会改变助手的推理方式。',
  ...
  'settings.persona.soul.desc': '助手在每次对话中遵循的性格提示词。编辑会保存到你的工作区，并在下次回复时生效。',
  ...
  'settings.memoryWindow.balanced.hint': '合理的默认值——在不为每次运行消耗额外 token 的情况下保持良好的连贯性。',
  'settings.memoryWindow.balanced.label': '均衡',
  'settings.memoryWindow.description': 'OpenHuman 在每次新的智能体运行中注入多少记忆上下文。更大的窗口对过往对话更有感知，但每次运行会消耗更多 token，成本也更高。',
  ...
  'skills.install.errors.alreadyInstalledHint': '工作区中已存在此 slug 的技能。首先将其删除或更改 frontmatter `metadata.id` / `name`。',
  'skills.install.errors.alreadyInstalledTitle': '技能已安装',
  'skills.install.errors.fetchFailedHint': '请求未成功完成。检查 URL 指向可访问的公共文件，并且主机返回 2xx 响应。',
  ...
  'skills.install.errors.fetchTooLargeHint': 'SKILL.md 必须小于 1 MiB。将捆绑的资源拆分为“references/”或“scripts/”文件，而不是内联它们。',
  ...
  'skills.install.errors.invalidSkillHint': 'frontmatter 必须是有效的 YAML，具有非空的“name”和“description”字段，并以“---”结尾。',
  ...
  'skills.install.errors.unsupportedUrlHint': '只有直接的“.md”链接才有效。对于 GitHub，链接到文件 (github.com/owner/repo/blob/.../SKILL.md) - 未安装树和存储库根。',
  'skills.install.errors.unsupportedUrlTitle': '不支持 URL 形式',
  'skills.install.errors.writeFailedHint': '工作区技能目录不可写。检查“<workspace>/.open human/skills/”的文件系统权限。',
  ...
  'skills.install.successNoNewIds': '已安装技能，但没有出现新的技能 ID - 目录可能已包含具有相同 slug 的技能。',
  ...
  'skills.uninstall.description': '这将永久删除技能目录及其所有捆绑资源。智能体将在下一轮停止看到它。',
  ...
  'settings.localModel.status.manageOllamaExternal': '请在 OpenHuman 之外管理 Ollama 进程和模型拉取，然后重新运行诊断。',
  ...
  'vault.confirmRemovePurge': '删除存储库「{name}」？\n\n点击确定将同时清除其记忆（删除全部 {count} 个已导入文档）。\n点击取消将保留记忆中的文档。',
  ...
  'settings.taskSources.description': '从 GitHub、Notion、Linear 和 ClickUp 收集工作项，补充信息后路由到智能体待办板。',
  ...
  'settings.taskSources.removeConfirm': '移除此任务来源？所有已摄取的任务历史都将被删除，且无法撤销。',
  ...
  'settings.agents.editor.builtInReadonly': '内置智能体不可编辑。您可以在智能体列表中启用、禁用或重置它们。',
};
