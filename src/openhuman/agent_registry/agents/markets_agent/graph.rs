//! LangGraph-compatible execution chain for the `markets_agent` built-in agent.
//!
...
/// The `markets_agent` agent's execution chain.
pub fn graph() -> GraphBlueprint {
    blueprint::canonical_turn("markets_agent")
}
