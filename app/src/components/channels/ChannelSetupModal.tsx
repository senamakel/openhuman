...
function ChannelConfigContent({ definition }: { definition: ChannelDefinition }) {
  ...
  switch (channelId) {
    ...
    default:
      return (
        <p className="text-sm text-stone-400 dark:text-neutral-500 dark:text-neutral-500 py-4">
...
export default function ChannelSetupModal({ definition, onClose }: ChannelSetupModalProps) {
  ...
  const modalContent = (
    <div
      ...
      <div
        ref={modalRef}
        className="bg-white dark:bg-neutral-900 dark:bg-neutral-900 border border-stone-200 dark:border-neutral-800 dark:border-neutral-800 rounded-3xl shadow-large w-full max-w-[500px] overflow-hidden animate-fade-up focus:outline-none focus:ring-0"
        style={{
        ...
        {/* Header */}
        <div className="px-5 pt-5 pb-4 border-b border-stone-200 dark:border-neutral-800 dark:border-neutral-800">
          <div className="flex items-start justify-between">
            <div className="flex-1 min-w-0 pr-2">
              <div className="flex items-center gap-2">
                {icon && <span className="text-base">{icon}</span>}
                <h2 id="channel-setup-title" className="text-base font-semibold text-stone-900 dark:text-neutral-100 dark:text-neutral-100">
              ...
              <p className="text-xs text-stone-500 dark:text-neutral-400 dark:text-neutral-400 mt-1.5">{definition.description}</p>
            </div>
            <button
              onClick={onClose}
