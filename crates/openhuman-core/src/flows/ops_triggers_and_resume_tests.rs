...
async fn flows_create_binds_schedule_cron_job_for_an_enabled_flow() {
    ...
    let job = crate::cron::find_flow_schedule_job(&config, &created.value.id).unwrap();
    assert!(
        job.is_some(),
...
async fn reconcile_schedule_triggers_on_boot_survives_a_corrupt_row() {
    ...
    let good_job = crate::cron::find_flow_schedule_job(&config, &good.value.id)
        .unwrap()
        .expect("precondition: good flow's cron job bound on enable");
    crate::cron::remove_job(&config, &good_job.id).unwrap();
    assert!(
        crate::cron::find_flow_schedule_job(&config, &good.value.id)
            .unwrap()
            .is_none(),
...
async fn flows_update_rebinds_schedule_cron_job_when_trigger_schedule_changes() {
    ...
    flows_set_enabled(&config, &created.value.id, true)
        .await
        .unwrap();
    let old_job = crate::cron::find_flow_schedule_job(&config, &created.value.id)
        .unwrap()
    ...
    let new_job = crate::cron::find_flow_schedule_job(&config, &created.value.id)
        .unwrap()
    ...
    let flow_jobs: Vec<_> = crate::cron::list_jobs(&config)
        .unwrap()
        .into_iter()
...
async fn flows_update_does_not_rebind_when_graph_is_not_supplied() {
    ...
    let job = crate::cron::find_flow_schedule_job(&config, &created.value.id)
        .unwrap()
