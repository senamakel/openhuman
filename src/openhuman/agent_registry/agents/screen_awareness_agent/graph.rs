//! LangGraph-compatible execution chain for the `screen_awareness_agent` built-in agent.
//!
...
/// The `screen_awareness_agent` agent's execution chain.
pub fn graph() -> GraphBlueprint {
    blueprint::canonical_turn("screen_awareness_agent")
}
