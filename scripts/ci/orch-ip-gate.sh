...
[ -d crates/openhuman-core/src/hosted/orchestration/graph ] && note "orchestration/graph/ (local wake graph) present"
for d in reasoning_agent frontend_agent master_agent master_reporter; do
  [ -d "crates/openhuman-core/src/hosted/orchestration/$d" ] && note "orchestration/$d agent package present"
done

# 3. Any orchestration prompt.md asset (the proprietary prompt IP).
if find crates/openhuman-core/src/hosted/orchestration -name 'prompt.md' -print -quit 2>/dev/null | grep -q .; then
...
if grep -rqE 'ProductionRuntime|run_orchestration_graph|OrchestrationRuntime|orchestration_graph_topology' \
  crates/openhuman-core/src/hosted/orchestration 2>/dev/null; then
  note "local wake-graph symbol present in orchestration/"
fi

# 5. Per-agent model-selection / tier-routing metadata.
if grep -rqE 'agent_tier|^\s*\[model\]' crates/openhuman-core/src/hosted/orchestration --include='*.toml' 2>/dev/null; then
