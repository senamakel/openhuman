      ...
      <div className="p-4 space-y-4">
        <div>
          ...
          <div className="bg-white dark:bg-neutral-900 rounded-xl border border-stone-200 dark:border-neutral-800 overflow-hidden">
            {available.length === 0 ? (
              <p className="p-4 text-sm text-stone-500 dark:text-neutral-400">{t('settings.mascot.noColorVariants')}</p>
            ) : (
              <div
                    ...
                    <button
                      ...
                      <span
                        className={`w-10 h-10 rounded-full border-2 transition-shadow ${
                          selected ? 'border-primary-500 shadow-soft' : 'border-stone-200 dark:border-neutral-800'
                        }`}
                        style={{ backgroundColor: palette.bodyFill }}
                      />
                      <span className="text-xs text-stone-700 dark:text-neutral-200">{opt.label}</span>
                    </button>
                  );
                })}
              </div>
        ...
        <div>
          ...
          <div className="bg-white dark:bg-neutral-900 rounded-xl border border-stone-200 dark:border-neutral-800 overflow-hidden">
            ...
            {!backendListError && backendList === null && (
              <p className="p-4 text-sm text-stone-500 dark:text-neutral-400">{t('settings.mascot.loadingLibrary')}</p>
            )}
            {backendList && backendList.length === 0 && !backendListError && (
              <p className="p-4 text-sm text-stone-500 dark:text-neutral-400">{t('settings.mascot.noCharacters')}</p>
            )}
