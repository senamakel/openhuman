//! LangGraph-compatible execution chain for the `integrations_agent` built-in agent.
//!
...
/// The `integrations_agent` agent's execution chain.
pub fn graph() -> GraphBlueprint {
    blueprint::canonical_turn("integrations_agent")
}
