  ...
  return (
    <div>
      ...
      <div className={embedded ? 'space-y-4' : 'p-4 space-y-4'}>
        <p className="text-stone-500 dark:text-neutral-400 text-sm">{t('settings.tools.chooseCapabilities')}</p>
            ...
            return (
              <div key={category}>
                <div className="mb-2">
                  ...
                  <p className="text-xs text-stone-400 dark:text-neutral-500">{CATEGORY_DESCRIPTIONS[category]}</p>
                </div>
                <div className="space-y-1">
                  {tools.map(tool => (
                    <button
                      ...
                      className="w-full flex items-center justify-between p-2.5 rounded-xl border border-stone-200 dark:border-neutral-800 bg-white dark:bg-neutral-900 hover:border-stone-300 dark:hover:border-neutral-700 transition-colors text-left">
                      <div className="min-w-0 flex-1">
                        <span className="text-sm font-medium text-stone-900 dark:text-neutral-100">
                          {tool.displayName}
                        </span>
                        <p className="text-xs text-stone-500 dark:text-neutral-400 mt-0.5">{tool.description}</p>
                      </div>
                      <div
                        className={`ml-3 flex-shrink-0 w-9 h-5 rounded-full transition-colors relative ${
                          enabled[tool.id] ? 'bg-sage-500' : 'bg-stone-200 dark:bg-neutral-700'
                        }`}>
                        <div
