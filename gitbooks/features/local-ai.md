...
| --- | --- | --- |
| **Memory embeddings** | `all-minilm:latest` | `src/openhuman/embeddings/ollama.rs` — used by the [Memory Tree](memory-tree.md) for vector search. |
...
- The provider router creates a *health-gated* local provider on startup. If Ollama is not reachable, requests transparently fall back to the remote provider — no broken state.
...
- [Memory Tree](memory-tree.md) — what local embeddings + summarization power.
