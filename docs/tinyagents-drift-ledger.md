...
| # | Area | Status | Evidence / action |
| --- | --- | --- | --- |
| P1-1 | `SchemaCleanr` provider schema normalization | **UPSTREAM PR OPEN** | TinyAgents PR [tinyagents#20](https://github.com/tinyhumansai/tinyagents/pull/20) ports `SchemaCleanr` into `harness::tool` with crate-native errors and ported tests. Host submodule remains pinned to `v1.6.0` until the upstream PR merges/releases; no host call-site deletion yet. Local validation in the submodule: `cargo fmt --check`; `timeout 180s cargo clippy --all-targets -- -D warnings`; `timeout 120s cargo test schema_`. |
| P1-2 | `current_time` / `resolve_time` builtin tool pilot | **UPSTREAM PR OPEN** | TinyAgents PR [tinyagents#22](https://github.com/tinyhumansai/tinyagents/pull/22) adds an optional `tools` feature and `harness::tools::{CurrentTimeTool, ResolveTimeTool, time_tools, register_time_tools}`. `chrono` / `chrono-tz` stay feature-gated and out of the default dependency set. Host wrappers remain in place until the upstream PR merges/releases and the host bumps the submodule. Local validation in the submodule: `cargo fmt --check`; `timeout 240s cargo clippy --features tools --all-targets -- -D warnings`; `timeout 180s cargo test --features tools time_`. |
| P1-3 | `model_context.rs` generic context-window patterns | **UPSTREAM PR OPEN** | TinyAgents PR [tinyagents#23](https://github.com/tinyhumansai/tinyagents/pull/23) adds `harness::context_window_for_model_id` as a provider-neutral fallback for common raw model ids, including substring matching for long family names and leading-segment matching for short `o1` / `o3` ids. The pattern table documents its specificity-first ordering invariant so broad entries such as `gpt-4` do not shadow narrower entries such as `gpt-4.1`. OpenHuman tier aliases and cost-catalog arms stay host-side. Host context-resolution call sites remain unchanged until the upstream PR merges/releases and the host bumps the submodule. Local validation in the submodule: `cargo fmt --check`; `timeout 180s cargo clippy --all-targets -- -D warnings`; `timeout 120s cargo test context_window_patterns_cover_common_provider_families`; `timeout 120s cargo test o1_o3_context_patterns_require_segment_boundaries`. |
| P1-4 | `error_classify.rs` generic provider failure classifiers | **UPSTREAM PR OPEN** | TinyAgents PR [tinyagents#24](https://github.com/tinyhumansai/tinyagents/pull/24) adds `harness::retry::{ProviderFailureClass, classify_provider_failure, classify_provider_error, structured_http_status, parse_retry_after_ms}` and wires the OpenAI adapter through the shared retryability classifier. Host-specific session, billing-envelope, and OpenHuman backend phrase rules stay host-side. Host retry/failure call-site swaps wait for the upstream PR to merge/release and the submodule to bump. Local validation in the submodule: `cargo fmt --check`; `timeout 180s cargo clippy --all-targets -- -D warnings`; `timeout 120s cargo test provider_failure`; `timeout 120s cargo test structured_http_status`; `timeout 120s cargo test retry_after_parser_accepts_integer_float_and_space_separators`; `timeout 120s cargo test classify_provider_error_reads_structured_error_fields`. |
...
| P1-6 | Git-worktree `WorkspaceIsolation` provider | **UPSTREAM PR OPEN** | TinyAgents PR [tinyagents#25](https://github.com/tinyhumansai/tinyagents/pull/25) adds `harness::workspace::GitWorktreeIsolation`, git worktree create/list/status/diff/remove helpers, and overlap detection with dependency-free real-git tests. OpenHuman's global event-bus emissions stay host-side; TinyAgents emits workspace events through its existing `prepare_workspace` / `cleanup_workspace` helpers. Host adapter deletion waits for upstream merge/release and a submodule bump. Local validation in the submodule: `cargo fmt --check`; `timeout 180s cargo clippy --all-targets -- -D warnings`; targeted worktree tests for create/list/status/diff/remove plus overlap and sanitize filters. |
| P1-7 | Tool display metadata and timeout semantics | **UPSTREAM PR OPEN** | TinyAgents PR [tinyagents#26](https://github.com/tinyhumansai/tinyagents/pull/26) adds `harness::tool::{ToolDisplay, ToolTimeout, humanize_tool_name, context_detail_from_args}` plus default `Tool::display_label`, `Tool::display_detail`, and `Tool::timeout_policy` hooks. Display metadata stays out of model-visible `ToolSchema`; `ToolPolicy` deserializes old JSON without the new display field. Host tool-trait call-site swaps wait for upstream merge/release and a submodule bump. Local validation in the submodule: `cargo fmt --check`; `timeout 180s cargo clippy --all-targets -- -D warnings`; `timeout 120s cargo test display_`; `timeout 120s cargo test tool_policy_deserializes_without_display_metadata`; `timeout 120s cargo test timeout_policy_uses_richer_timeout_semantics`. |
